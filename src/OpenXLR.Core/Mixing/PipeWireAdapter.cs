...
public sealed class PipeWireAdapter
{
    ...
    {
        if (inserts?.Any(i => !i.Bypass && (i.RunsNatively || NeedsLatencyHost(i) || UsesNativeLv2Fallback(i))) == true)
        ...
        int lowCutHz, bool clipGuard, IReadOnlyList<InsertDefinition> inserts, bool forceNativeLv2 = false)
    {
        ...
        try
        {
            ...
            foreach (InsertDefinition insert in inserts.Where(i => !i.Bypass))
            {
                ...
                FilterHandle stage;
                if (insert.RunsNatively || NeedsLatencyHost(insert) || forceNativeLv2 && insert.Kind == "lv2" || UsesNativeLv2Fallback(insert))
                {
        ...
        bool exposeHardwareMonitorOutputs = false, string? hardwareSinkHint = null)
    {
        ...
        foreach (JsonElement o in GraphObjects())
        {
            ...
            if (name.StartsWith("OpenXLR_bus_", StringComparison.Ordinal) || name.StartsWith("OpenXLR_route_", StringComparison.Ordinal) || name.StartsWith("OpenXLR_delay_", StringComparison.Ordinal) || name.StartsWith("OpenXLR_soundcheck_", StringComparison.Ordinal)) continue;
