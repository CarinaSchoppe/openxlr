...
static void tick(void *data, uint64_t expirations) {
  Host *h = data;
  // Both the UI/main loop and the active audio callback must make progress.
  // An idle, unlinked plugin is healthy without processing audio buffers.
  if (++h->heartbeat_ticks == 30) {
    uint64_t cycles = atomic_load(&h->completed_cycles);
    if (!h->streaming || cycles != h->last_cycles)
      puts("heartbeat");
    h->last_cycles = cycles;
    h->heartbeat_ticks = 0;
  }
...
int main(int argc, char **argv) {
  ...
  if (channels < 1 || channels > 2 || h.rate < 8000 || h.rate > 384000)
    return 2;
  // A daemon SIGKILL must not leave orphan DSPs connected to its graph.
